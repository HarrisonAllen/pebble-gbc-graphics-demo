...
GBC_Graphics *GBC_Graphics_ctor(Window *window) { 
  ...
  *(GBC_Graphics * *)layer_get_data(self->sprite_layer) = self;
  ...
  self->vram = (uint8_t*)malloc(VRAM_BANK_SIZE * 1);
...
static void render_bg_graphics(GBC_Graphics *self, Layer *layer, GContext *ctx) {
  ...
  for (self->line_y = 0; self->line_y < self->screen_height; self->line_y++) {
    ...
    for(x = min_x; x < max_x; x++) {
      ...
      map_tile_x = ((map_x >> 3) + MAP_WIDTH) % MAP_WIDTH; // map_x / TILE_WIDTH
      map_tile_y = ((map_y >> 3) + MAP_HEIGHT) % MAP_HEIGHT; // map_y / TILE_HEIGHT

      // Get the tile and attrs from the map
      tile_num = tilemap[map_tile_x + (map_tile_y * MAP_WIDTH)]; // map_tile_y * MAP_WIDTH
      tile_attr = attrmap[map_tile_x + (map_tile_y * MAP_WIDTH)]; // map_tile_y * MAP_WIDTH
    ...
    for (tile_num = 0; tile_num <= sprite_mode; tile_num++) {
      ...
      for (tile_y = 0; tile_y < TILE_HEIGHT; tile_y++) {
        ...
        for (tile_x = 0; tile_x < TILE_WIDTH; tile_x++) {
          ...
          bg_tile_attr = self->bg_attrmap[map_tile_x + (map_tile_y * MAP_WIDTH)]; // map_tile_y * MAP_WIDTH
          ...
          if (sprite[3] & ATTR_PRIORITY_FLAG) {
            bg_tile_num = self->bg_tilemap[map_tile_x + (map_tile_y * MAP_WIDTH)]; // map_tile_y * MAP_WIDTH
