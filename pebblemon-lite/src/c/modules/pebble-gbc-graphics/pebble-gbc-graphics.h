...
#define MAP_WIDTH 24  ///> Width of the background and window maps in tiles
#define MAP_HEIGHT 24 ///> Height of the background and window maps in tiles
/**
 * Size of the tilemap in bytes, calculated by:
 * 1 byte per tile location * 32 tiles wide * 32 tiles tall = 1024 bytes
 */
#define TILEMAP_SIZE (MAP_WIDTH * MAP_HEIGHT)
/**
 * Size of the attributemap in bytes, calculated by:
 * 1 byte per attribute * 32 tiles wide * 32 tiles tall = 1024 bytes
 */
#define ATTRMAP_SIZE  (MAP_WIDTH * MAP_HEIGHT)
/**
...
#define POINT_TO_OFFSET(x, y) ((x) % MAP_WIDTH) + ((y) % MAP_HEIGHT) * MAP_WIDTH ///> Converts an x, y point on a bg/window map to the tile/attrmap offset
