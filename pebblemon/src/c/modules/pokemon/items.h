...
const int16_t items[][6] = {
    {1, 19, 18, ITEM_ID_RUNNING_SHOES, false}, // Running Shoes (double move speed) - Forest, top half
    {1, 29, 26, ITEM_ID_CUT, false}, // Cut (enables cutting of trees) - Forest, bottom half
    {2, 37, 15, ITEM_ID_LUCKY_EGG, false}, // Lucky Egg (double exp) - National Park, top right
    {3, 12, 6, ITEM_ID_BERRY, false}, // Berry (heals 10HP when below half, one time use(?)) - Berry tree route 1, can come back for more(?)
    {2, 32, 42, ITEM_ID_LEFTOVERS, true}, // Leftovers (restore a 1/16 HP each turn) - Far right trash can National Park
    {0, 19, 23, ITEM_ID_FOCUS_BAND, false}, // Focus Band (12% chance to prevent fainting) - Cave, in path 
    {3, 48, 6, ITEM_ID_PROTEIN, false}, // Protein (Raises attack stat by 25% each battle) - Route 1, top right
    {0, 7, 21, ITEM_ID_IRON, false}, // Iron (Raises defense stat by 25% each battle) - Cave, on stairs
};
